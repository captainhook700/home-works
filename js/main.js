// Написать функции для получения доступа к последнему пункту меню
// (App Design) различными способами:
// 1. По id (2 способа)

var lastEl = document.getElementById('mobile');
console.log('id: ', lastEl);
lastEl = document.querySelector('#mobile');
console.log('id: ', lastEl);

// Используя класс items-nav родительского элемента (требуется несколько действий)
var parentItem = document.getElementsByClassName('items-nav')[0];
var listOfLi = parentItem.getElementsByTagName('li');
lastEl = listOfLi[listOfLi.length-1].getElementsByTagName('a')[0];
console.log(lastA);

// Используя исключительно имена тегов (требуется несколько действий)
...
// Используя селекторы (в одно действие) - по id, по классу
// items-nav родителя, используя только теги.
// При получении доступа поменять элементу цвет шрифта и вывести в
// консоль его содержимое.
...
// Написать функцию, при помощи которой мы можем посчитать, сколько
// раз на странице используется элемент.
// Пример:
// Функция принимает 2 параметра:
// 1. DOM ссылка на элемент, с которого начинаем поиск (например если
// хотим начать с корня, то можно передать document.body или
...
// 2. Внутри функции, получаем количество потомков изначального
// элемента.
// 3. Создаём цикл, где количество итераций равно количеству потомков.
// 4. Проверяем - если текущий элемент соответствует искомому,
// увеличиваем счётчик на 1.
// 5. Далее проверяем, если у текущего элемента есть дети, снова
// вызываем нашу функцию (внутри самой себя), но уже передаём в неё
// текущий элемент в качестве изначального
// 6. В конце возвращаем результат счётчика (после этого можно например
// вывести его в консоль). (edited)
